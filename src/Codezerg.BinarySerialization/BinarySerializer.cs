...
public static class BinarySerializer
{
    ...
    private static object? ReadValue(BinarySerializationReader reader, Type targetType, SerializerOptions options)
    {
        ...
        if (underlyingType != null)
        {
            return ReadValue(reader, underlyingType, options);
        }
        ...
        {
            return ReadArray(reader, targetType, options);
        }

        // Dictionary
        if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(Dictionary<,>))
        {
            return ReadDictionary(reader, targetType, options);
        }

        // List
        if (targetType.IsGenericType && targetType.GetGenericTypeDefinition() == typeof(List<>))
        {
            return ReadList(reader, targetType, options);
        }

        // Complex object
        if (valueType == SerializedType.Map)
        {
            return ReadObject(reader, targetType, options);
        }

        throw new InvalidOperationException($"Cannot deserialize type {targetType.Name} from {valueType}");
    }

    private static Array ReadArray(BinarySerializationReader reader, Type arrayType, SerializerOptions options)
    {
        ...
        {
            array.SetValue(ReadValue(reader, elementType, options), i);
        }
        return array;
    }

    private static object ReadList(BinarySerializationReader reader, Type listType, SerializerOptions options)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            list.Add(ReadValue(reader, elementType, options));
        }
        return list;
    }

    private static object ReadDictionary(BinarySerializationReader reader, Type dictType, SerializerOptions options)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            var key = ReadValue(reader, keyType, options);
            var value = ReadValue(reader, valueType, options);
            dict.Add(key!, value);
        }
        return dict;
    }

    private static object ReadObject(BinarySerializationReader reader, Type type, SerializerOptions options)
    {
        var typeInfo = GetTypeInfo(type, options);
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (member != null)
            {
                ...
                {
                    var value = ReadValue(reader, member.MemberType, options);
    ...
    private sealed class TypeInfo
    {
        ...
        public TypeInfo(Type type, SerializerOptions options)
        {
            ...
            foreach (var prop in type.GetProperties(BindingFlags.Public | BindingFlags.Instance))
            {
                ...
                members.Add(new MemberInfo(name, prop.PropertyType, order,
            ...
            if (includeFields)
            {
                foreach (var field in type.GetFields(BindingFlags.Public | BindingFlags.Instance))
                {
                    ...
                    members.Add(new MemberInfo(name, field.FieldType, order,
            ...
            Members = members;
            MembersByName = members.ToDictionary(m => m.Name);
        }
    }
    ...
    private sealed class MemberInfo
    {
        ...
        public MemberInfo(string name, Type memberType, int order,
